/*
	...
	heb nu een soort van bal
	waar ik wil doen wat RK doet
	Wat jeroen zegt:
	1e video is hij heeft een model,
	als ray project op die bal
	dus hij projecteerd die vertices op het model vanaf de Y Axis en de Z Axis bv
	en volgens mij projecteerd hij die alpha mask (2d en geanimeerd)
	denk dat het model er minder toe doet dan de alpha masks die je gebruikt om te raycasten
	en hij heeft twee alpha masks (2d)

	Tokyo
	ja stel je voor je hebt een plane
	een square
	grid
	elke vertex schiet je met een rechte lijn richting je model
	en waar ze de vertex het model raakt daar stop het
	dan de volgende vertex, schiet op model, en stop
	etc. etc.
	todat je de hele grid naar het model hebt geschoten
	logischerwijs worden neit alle punten op het model geschoten omdat soms er geen 'intersection' is
...
function render(dt) {
	...
	// zoom in and out
	// controls.distance = Map(Math.sin(Math.PI * time)+1, 1, 2, 150, 200)

	if (knot) scene.remove(knot)
	var knotR = Math.sin(Math.PI * (lowAvg+t)) + SETTINGS.radius
	...
	geom = new THREE.TorusKnotGeometry(
					...
					Math.round(Math.sin(Math.PI * t)), 
					Math.round(Math.cos(Math.PI * t))
					)
	...
	knot.rotation.x = Math.sin(Math.PI * 10) + t
	knot.rotation.y = Math.cos(Math.PI * 7.5) + t
	knot.rotation.z += .005
